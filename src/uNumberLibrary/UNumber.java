...
/**
 * <p>
 * Title: UNumber Class, a component of the Unlimited Precision Math Package.
 * </p>
 * 
 * <p>
...
public class UNumber implements Comparable<UNumber> {
	...
	/**
	 * An unlimited precision floating point value in this package is implemented
	 * using <B>decimal</B> arithmetic. Values are stored in scientific notation
	 * (i.e. mantissa x 10 to the characteristic power. The "mantissa" is a sequence
	 * of 0 to 9 digits with the assumed decimal point to the left of the Most
	 * Significant Digit (MSD) in the mantissa.
	 * 
	 * The "characteristic" is an integer value. This determines the location of the
	 * decimal point and performs the equivalent of multiply the mantissa by ten
	 * raised to the power of the characteristic
	 * 
	 * After any computation, the package ensures that the result is "normalized".
	 * This means that with the exception of a result value of zero, the MSD will be
	 * in the range "1" to "9". We do this to speed future computations.
	 * Normalization discards leading zeros in the result and for each zero from the
	 * left side that is removed, the characteristic is reduced by one to
	 * compensate.
	 */
	...
	/**********
	 * This constructor creates a variable length number based on an integer input
	 * value
	 * 
	 * @param v if the value is less than 0 than it is false.
	 */
	...
	/**********
	 * This constructor creates a variable length number based on a long integer
	 * input value
	 * 
	 * @param v if the value is less than 0 than it is false.
	 */
	...
	/**********
	 * This constructor creates a number based on a digit sequence in a string, a
	 * power of ten, and a sign
	 * 
	 * @param str  The string specifies the digit sequence and it's length is the
	...
	/**********
	 * This constructor creates a number based on a digit sequence in a string, a
	 * power of ten, a sign, and a size constant. If the size is larger than the
	 * constant sequence, the extra positions are filled with zero. If the size is
	 * less than the digit sequence, the excess digits are discarded.
	 * 
	 * @param str  The string specifies the digit sequence and it's length is the
	 *             number of significant digits
	 * 
	...
	public UNumber(String str, int dec, boolean sign, int size) {
		...
		for (int i = Math.min(str.length(), size); i < size; i++)
			d[i] = 0;
	 ...
	 * 
	 * @param that This parameter is used to initialize the number.
	 * @param size this parameter is a int or a variable value which assign as size.
	 */
	public UNumber(UNumber that, int size) {
		d = new byte[size];
		for (int i = 0; i < Math.min(that.d.length, size); i++)
			d[i] = that.d[i];
		for (int i = Math.min(that.d.length, size); i < size; i++)
			d[i] = 0;
		dP = that.dP;
		setS(that.isS());
	}
	...
	/**********
	 * This constructor creates a potentially unnormalized value (there maybe
	 * leading zeros) with enough space in the mantissa array to hold all of the
	 * significant digits of both input parameters at the same time. It initializes
	 * the new number to the value of the first parameter. This constructor is used
	 * to align the decimal points of two numbers.
	 * 
	 * @param that    This parameter is used to initialize the number
	 * 
	 * @param another This one helps determine the how many significant digits to
	 *                use
	 */
	public UNumber(UNumber that, UNumber another) {
		...
		// The third loop fill in any trailing zeros (if needed) to fill the mantissa
		// array
		for (int i = index; i < left + right; i++)
			d[i] = 0;
	...
	/**********
	 * This constructor takes a double and creates a UNumber from it. The result is
	 * rounded to a 14 significant digit result.
	 * 
	 * @param v it will give us if the value -ve than it give false
	 */
	public UNumber(double v) {
		// Allocate enough space to hold 14 significant digits plus one for rounding and
		// one
		...
		// One by one, peel off the most-significant digit and place it into the byte
		// array
		...
		// If the last believable digit (the 16th) is greater than or equal to 5, round
		// up the
		// remaining 15 digits.
		if (tempD[15] >= 5) {
			// Round up
		...
		// If necessary, normalize the rounded 15 digit number... the value could still
		// be
		// 0.0999999
		if (tempD[0] == 0) {
	...
	/**********
	 * This implementation of toString converts the value to a String in scientific
	 * notation
	 */
	public String toString() {
		...
		result += "E";
		if (dP < 0)
			result += dP; // If the result is negative, the sign will automatically be
							// inserted
		else
			result += "+" + dP; // For positive values, we must manually insert the "+" sign
		return result;
	}

	/**********
	 * This implementation of toString converts the value to a String in decimal
	 * notation as long as the exponent is in the range +8 to -6
	 * 
	 * @return This returns the value of result.
	 */
	...
	/**********
	 * If the first digit of the UNumber is zero, the value must be zero as these
	...
	/**********
	 * This implementation of toString converts the value to a String in using a
	 * decimal notation
	 * 
	 * @return This returns the value of result.
	 */
	public String toDecimalString() {
		...
		// For values less than one, we will display one zero to the left of the decimal
		// point
		if (dP <= 0)
			result += "0";

		// For values less than 1/10, we need to insert zeros between the decimal point
		// and
		// the most significant digit
		if (dP < 0) {
			result += "."; // Insert the decimal point
			for (int i = 0; i > dP; i--)
				result += "0"; // Insert the zeros
			for (int i = 0; i < d.length; i++)
				result += d[i]; // Insert the significant digits
		} else {
			// For values greater than 1/10, we insert the decimal point at the appropriate
			// point
			// as we insert in the digits (if dP <= d.length)
			for (int i = 0; i < d.length; i++) {
				if (i == dP)
					result += "."; // Insert the decimal point at the right place
			...
			if (dP > d.length)
				for (int i = d.length; i < dP; i++)
					result += "0"; // Insert the zeros
			if (dP >= d.length)
				result += "."; // and then the decimal point
		}
		return result;
	}

	/**********
	 * This routine is used to display big numbers on the console, 80 characters per
	 * line with the characteristic displayed on the last line (which might be
	 * longer than 80 characters in length).
	 */
	...
	/*********
	 * This routine displays just the first 10 significant digits of a UNumber
	 * followed by the characteristic to System.out (the console).
	 */
	public void displayShort() {
		if (!isS())
			System.out.print("-");
		for (int i = 0; i < Math.min(d.length, 10); i++)
	...
	private void normalize(UNumber that) {
		if (that.d[0] == 0) {
			...
			while (zeroIndex < that.d.length - 1 && that.d[zeroIndex] == 0)
				zeroIndex++;
			...
			// Create a new mantissa that has the appropriate number of significant digits
			// (at
			// least one)
			byte[] newD = new byte[that.d.length - zeroIndex];

			// Move the significant digits from the result mantissa array into the new
			// mantissa
			// array
			for (int i = zeroIndex; i < that.d.length; i++)
				newD[i - zeroIndex] = that.d[i];
	...
	public void add(UNumber that) {
		...
		if (this.isS() == that.isS()) {
			...
			if (borrow) {
				...
				temp1.setS(true); // Since the borrow was false at the end, the positive value
	 ...
	 * 
	 * @param that The second operand that is subtracted from the value of this
	 *             object's value
	 */
	...
	/**********
	 * This lessThan routine tries to avoid doing a subtraction in order to do
	 * things faster. Therefore checks of sign bits, then the decimal points, and
	 * then the first digits are done before falling into a subtract as a last
	 * resort. If one value is negative and the other is positive, there is no need
	 * to go any further. If they are of the same size, but the characteristics of
	 * the normalized values are not the same, there is no need to go any further.
	 * Given the same size and the same characteristic for normalized numbers, if
	 * the first digit can resolve the issue, try that. Only in the case where the
	 * signs, characteristics, *and* the first digits are the same do we resort to
	 * subtraction and a check of the sign bit.
	 * 
	...
	/**********
	 * This greaterThan routine tries to avoid doing a subtraction in order to do
	 * things faster. This code follows the same process as the less than code above
	 * 
	...
	/**********
	 * This absolute value routine sets the sign bit true which has the same effect
	 * of doing an absolute value on this
	 */
	public void abs() {
		setS(true);
	}
	...
	public double getDouble() {
		/**
		 *
		 * This method will calculate the square root of the give values which are in
		 * the ExperimentWithDoublesAndUNumbers class.
		 * 
		 * The formula used in this calculation is {((ValueB/ValueC)+ValueC)/2= ValueB}
		 */
		int numDigits = 17; // Doubles are not accurate beyond 17 digits
		if (d.length < 17)
			numDigits = d.length; // If the value has fewer, use just that many
	...
	public void sqrt() {
		UNumberWithSqrt ValueA = new UNumberWithSqrt(this); // take the value from the
															// ExperimentWithDoublesAndUNumber.java
															// Create Variable number store value of x.
		UNumberWithSqrt divisor = new UNumberWithSqrt(2); // Create a variable 2
		UNumberWithSqrt ValueB = new UNumberWithSqrt(ValueA); // Create a variable "ValueB" and store value of x.
		UNumberWithSqrt ValueC = new UNumberWithSqrt(ValueA); // Create a variable "ValueC" and store value of x.
		if (this.isS() == true) {
			int z = 0;
			ValueB.div(divisor);
			ValueC.div(ValueB);
			...
			ValueC.div(divisor);
			/*
			 * This loop if for to calculate the number of iteration for the give value
			 */
			while (z < 15) {
				ValueB = new UNumberWithSqrt(ValueA);
				ValueB.div(ValueC); // ValueB is Divided by ValueC
				ValueB.add(ValueC); // ValueB is Add in the ValueC
				ValueB.div(divisor); // ValueB is again divided by 2
				ValueC = new UNumberWithSqrt(ValueB); // ((ValueB/ValueC)+ValueC)/2 = ValueB
				z++;
